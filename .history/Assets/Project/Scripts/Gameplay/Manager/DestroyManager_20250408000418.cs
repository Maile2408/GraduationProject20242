...
public class DestroyManager : SaiBehaviour
{
    ...
    protected override void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                Debug.Log("[DestroyManager] Clicked but no valid target. Staying in destroy mode.");
            }
        }
    }
    ...
    private void UpdateRaycastTarget()
    {
        ...
        Debug.DrawRay(ray.origin, ray.direction * 1000f, Color.red, 0.2f);
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, 1000f, destroyableMask))
        {
            ...
            BuildDestroyable destroyable = hit.collider.GetComponent<BuildDestroyable>();
            if (destroyable == null) destroyable = hit.collider.GetComponentInParent<BuildDestroyable>();
            if (destroyable == null) destroyable = hit.collider.GetComponentInChildren<BuildDestroyable>();
