...
public class FarmTask : BuildingTask
{
    ...
    protected virtual void PlanWatering(WorkerCtrl workerCtrl)
    {
        ...
        for (int i = wateringStep; i < workingPoints.Count; i++)
        {
            Transform target = workingPoints[i];
        ...
        wateringStep = workingPoints.Count;
    }
    ...
    protected virtual void PlanHarvest(WorkerCtrl workerCtrl)
    {
        ...
        for (int i = grainIndex; i < grains.Count; i++)
        {
            Transform target = grains[i].transform;
            workerCtrl.workerMovement.SetTarget(target);
    ...
    IEnumerator Watering(WorkerCtrl workerCtrl)
    {
        ...
        if (wateringStep >= workingPoints.Count && phase != FarmPhase.Growing)
        {
