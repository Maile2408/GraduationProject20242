...
public class AbstractConstruction : SaiBehaviour
{
    ...
    [SerializeField] BuildingCtrl builder;
    ...
    {
        if (this.info == null || this.info.prefab == null)
        {
            Debug.LogWarning("[AbstractConstruction] Finish failed: Missing info or prefab");
            return;
        }

        GameObject realBuilding = PoolManager.Instance.Spawn(PoolPrefabPath.Building(info.prefab.name));
