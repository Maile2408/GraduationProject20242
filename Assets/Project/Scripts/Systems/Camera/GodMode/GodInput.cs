...
public class GodInput : SaiBehaviour
{
    ...
    protected virtual void HandleClickOnBuilding()
    {
        if (DestroyManager.Instance != null && DestroyManager.Instance.IsInDestroyMode()) return;
        
        Ray ray = godModeCtrl._camera.ScreenPointToRay(Input.mousePosition);

        if (Physics.Raycast(ray, out RaycastHit hit, 999f, buildingLayer))
        {
            var building = hit.collider.GetComponentInParent<BuildingCtrl>();
            if (building == null) return;

            BuildingInfoUIController.pendingBuilding = building;
            ScreenManager.Add<BuildingInfoUIController>(BuildingInfoUIController.NAME);
        }
    }
}